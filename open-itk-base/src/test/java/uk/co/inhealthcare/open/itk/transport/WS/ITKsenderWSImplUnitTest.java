...
import uk.co.inhealthcare.open.itk.transport.WS.ITKSenderWSImpl;

public class ITKsenderWSImplUnitTest extends TestCase {
	...
	public void testAuditFailure()  {
		...
		ITKTransportRouteImpl dest = new ITKTransportRouteImpl("WS", SERVICE_ENDPOINT);
		...
		try {
			...
			assertTrue(auditEx.getMessage().contains("Error Auditing SOAP Request"));
		}
		...
		try {
			...
			assertTrue(auditEx.getMessage().contains("Error Auditing SOAP Response"));
		}
	...
	public void testAudit() throws ITKMessagingException {
		...
		assertTrue(auditService.parm_requestAuditType.get(0).equals(AuditService.SOAPREQUEST));
		assertTrue(auditService.parm_requestAuditType.get(1).equals(AuditService.SOAPRESPONSE));
		SOAPAuditDetails soapRequestAudit = (SOAPAuditDetails) auditService.parm_requestAuditDetails.get(0);
		assertTrue(soapRequestAudit!=null);
		// Only conversation Id and Transport details are audited for the SOAP Request
		assertTrue(soapRequestAudit.getConversationId().equals(request.getConversationId()));
		assertTrue(soapRequestAudit.getCreationTime()!=null);
		assertTrue(soapRequestAudit.getMessageId().length()==36);
		assertFalse(soapRequestAudit.getMessageId().equals(request.getConversationId()));
		...
		SOAPAuditDetails soapResponseAudit = (SOAPAuditDetails) auditService.parm_requestAuditDetails.get(1);
		assertTrue(soapResponseAudit!=null);
		assertTrue(soapResponseAudit.getConversationId().equals(request.getConversationId()));
		assertTrue(soapResponseAudit.getAction().equals(SERVICE_ID+"Response"));
		assertTrue(soapResponseAudit.getStatus().equals("OK"));
		assertTrue(soapResponseAudit.getMessageId().length()==36);
		// Transport id on the response should not be the same as the conversation id 
		assertFalse(soapResponseAudit.getMessageId().equals(request.getConversationId()));
		// Transport id on the response should not be the same as the outbound transport id 
		assertFalse(soapResponseAudit.getMessageId().equals(soapRequestAudit.getMessageId()));
	...
	public void testSoapActionAndEndpoint() throws ITKMessagingException {
		...
		request.setMessageProperties(props);
		ITKTransportRouteImpl dest = new ITKTransportRouteImpl("WS", serviceEndpoint);
	...
	public void testResponseFormat() throws ITKMessagingException {
		...
		assertTrue(response.getConversationId().equalsIgnoreCase(request.getConversationId()));
	...
	public void testBusy() throws ITKMessagingException {
		...
		request.setMessageProperties(props);
		ITKTransportRouteImpl dest = new ITKTransportRouteImpl("WS", "http://localhost:8080/itk");
		try {
			wsSender.sendSync(dest,  request);
