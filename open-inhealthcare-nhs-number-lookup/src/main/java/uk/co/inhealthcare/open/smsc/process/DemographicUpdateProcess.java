...
/**
 * Process class to examine the in-bound demographic update message and use configuration to
 * decide if an SMSP call is required. If so then this process knows which SMSP function
 * to use acts upon the response.
 *
 * NOTE: Much of this is common with NewRegistrationProcess and could therefore be re-factored. 
...
@Component
public class DemographicUpdateProcess extends SMSCProcess {

	private final static Logger logger = LoggerFactory.getLogger(DemographicUpdateProcess.class);
	
	// INJECTED DEPENDENCIES
	private GetNHSNumberOperation getNHSNumberOperation;
	private VerifyNHSNumberOperation verifyNHSNumberOperation;
	//

	public DemographicUpdateProcess(){
		logger.debug("Creating DemographicUpdate Processor.");
	}
	...
	protected String processDetail(DemographicUpdate update, MessageProperties props) 
		...
		try {
			if (update.getNHSNumber().length()==0){
				// No NHS Number provided - perform Get NHS Number
				logger.debug("Performing Get NHS Number");
				GetNHSNumberRequest getNhsNumberRequest = update
						.createGetNHSNumberRequest(props.getConversationId());
				getNhsNumberRequest.setLocalAuditId(this.localAuditId);
				smspResponse = getNHSNumberOperation.process(getNhsNumberRequest);
			} else {
				// NHS Number is provided - perform Verify NHS Number
				logger.debug("Performing Verify NHS Number");
				VerifyNHSNumberRequest verifyNhsNumberRequest = update
						.createVerifyNHSNumberRequest(props.getConversationId());
				verifyNhsNumberRequest.setLocalAuditId(this.localAuditId);
				smspResponse = verifyNHSNumberOperation.process(verifyNhsNumberRequest);
			}
	...
	public void setGetNhsNumberOperation(GetNHSNumberOperation getNhsNumberOperation) {
		logger.debug("Setting GetNHSNumberOperation to:"+getNhsNumberOperation);
		this.getNHSNumberOperation = getNhsNumberOperation;
	}
	public void setVerifyNhsNumberOperation(VerifyNHSNumberOperation verifyNHSNumberOperation) {
		logger.debug("Setting VerifyNHSNumberOperation to:"+verifyNHSNumberOperation);
		this.verifyNHSNumberOperation = verifyNHSNumberOperation;
	}
