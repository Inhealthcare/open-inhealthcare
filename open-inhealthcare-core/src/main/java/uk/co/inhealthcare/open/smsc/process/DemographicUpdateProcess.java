	...
	protected String processDetail(DemographicUpdate update, MessageProperties props) 
		...
		try {
			if (update.getNHSNumber().length()==0){
				...
				GetNHSNumberRequest getNhsNumberRequest = new GetNHSNumberRequest(update, props.getConversationId());
				...
				VerifyNHSNumberRequest verifyNhsNumberRequest = new VerifyNHSNumberRequest(update, props.getConversationId());
