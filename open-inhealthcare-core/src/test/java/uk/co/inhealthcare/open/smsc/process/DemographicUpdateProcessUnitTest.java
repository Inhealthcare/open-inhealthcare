...
import uk.co.inhealthcare.open.smsc.operation.GetNHSNumberOperation_Mock;
import uk.co.inhealthcare.open.smsc.operation.VerifyNHSNumberOperation_Mock;
import uk.co.inhealthcare.open.smsc.process.DemographicUpdateProcess;
import uk.co.inhealthcare.open.smsc.process.SMSCProcess;
 ...
 * 
 * Responsibilities
 * - Accept a Demographic Update (A31)
 ...
 *  
 */
public class DemographicUpdateProcessUnitTest extends TestCase {
	...
	public void testConfigMissingSMSCLogger()  {
		...
		DemographicUpdateProcess process = getConfiguredProcess(SMSP_OK);
	...
	public void testRequestLoggingInputException() throws ConfigurationException, ValidationException {
		...
		assertTrue(techAlerter.businessProcess.equals("DemographicUpdateProcess"));
	...
	public void testLoggingOk() throws ConfigurationException, ValidationException {
		...
		// Default request has no NSTS Status code and therefore has no SMSP activity requirement
		process.process(request);
		...
		assertTrue(logger.parm_Outcome.get(0).equals(DemographicUpdateProcess.NO_SMSP_LOOKUP_REQUIRED));
	...
	private void testValidRequest(String inStatus, String inNhsNumber, String outStatus, String outNhsNumber, String smspResponseRequired) throws ConfigurationException, ValidationException, HL7Exception, TransformationException {
		...
		DemographicUpdateProcess process = getConfiguredProcess(smspResponseRequired);
		ADT_A05 request = getValidRequest(inStatus,inNhsNumber);
		...
		boolean businessAlertRequired = process.getLookupBoolean("SMSP.ResponseCodeBusinessAlert.A31",smspResponseRequired,"N");
	...
	// UTILS
	private DemographicUpdateProcess getConfiguredProcess(String smspResponse){
		...
		DemographicUpdateProcess process =  new DemographicUpdateProcess();
		...
		VerifyNHSNumberOperation_Mock mockVerify = new VerifyNHSNumberOperation_Mock();
		mockVerify.primeResponse(smspResponse, primeNhsNumber, primeVerified, "");
		process.setVerifyNhsNumberOperation(mockVerify);
		
		GetNHSNumberOperation_Mock mockGet = new GetNHSNumberOperation_Mock();
		mockGet.primeResponse(smspResponse, primeNhsNumber,"");
		process.setGetNhsNumberOperation(mockGet);
